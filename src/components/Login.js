...
const Login = () => {
  ...
  try{   await axios.post(url,data)
        .then((response)=>{  persistLogin(response.data.token,response.data.username,response.data.balance); setApiCall(false);  enqueueSnackbar("Logged in successfully")})
        .catch((err)=>{ console.log(err.response.data.message); setApiCall(false); enqueueSnackbar(err.response.data.message)})
  }catch(e){ enqueueSnackbar("Something went wrong please check backend") };
        ...
        <CircularProgress />
          ...
          <p className="secondary-action">
            Don't have an account?{" "}
             ...
             <Link className="link" to="/register" replace>Register Now</Link>
